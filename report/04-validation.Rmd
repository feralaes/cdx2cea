...
In this fourth component, we check our decision model's internal validity before we move on to the analysis components. We compared the model-predicted output evaluated at posterior parameters against the calibration targets to internally validate the decision model [@Alarid-Escudero2020b]. This is all done in the *04_validation.R* script in the `analysis` folder.

In section _04.2 Compute model-predicted outputs_, we compute the model-predicted outputs for each posterior distribution sample and the MAP estimate. We then use the function `data_summary` to summarize the model-predicted posterior outputs into different summary statistics. 
```{r}
